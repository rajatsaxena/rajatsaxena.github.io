...
- Load the raw LFP data along with the sampling rate
- Bandpass filter the data in `150-250 Hz`. There is some conflict in the field on what range to use. 
- Calculate the `RMS` power by convolving the signal with the kernel of `size=9`. 
- Calculate the `mean rms` and `std rms' power
- Set a `minimum and maximum ripple duration`. I have used 20ms and 200ms as the minimum and maximum SWR time.
- Set a ripple power threshold for finding the SWR peak. I used `mean_rms + 5*std_rms` as threshold.
- Decide a `falloff` threshold to calculate the ripple start time, end time and duration. In this case, 
`falloff_thresh = mean_rms + 0.5*std_rms` 
- For each ripple peak, calculate the ripple starttime, endtime, duration, and peaktime. 
- Remove putative ripple events which do not lie within the duration range. 

**Some additional comments for future analysis**
- The analysis results can be improved by looking at spiking data recorded from all the neurons. As mentioned earlier,
...
- Another way to improve SWR detection is by using LFP from multiple sites. 


<img src="https://github.com/rajatsaxena/rajatsaxena.github.io/blob/master/images/SWR.png" width="125%" height="125%">
